...
# ----------------------
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # Solution goes here
    users_with_item_8 = Order.joins(:items).where("items.id = #{@item_8.id}").pluck(:user_id)
    users = User.where(id: users_with_item_8).order(:name).pluck(:name)
    # ------------------------------------------------------------
  ...
  it '17. returns the name of items associated with a specific order' do
    ...
    # ------------------------------------------------------------
    # binding.pry
    names = Order.find("#{@order_12.id}").items.pluck(:name)
    # Expectation
  ...
  it '18. returns the names of items for a users order' do
    ...
    # Solution goes here
    # ------------------------------------------------------------
    items_for_user_3_third_order = User.joins(:orders).find("#{@user_3.id}").orders[2].items.pluck(:name)
    # Expectation
  ...
  it '20. returns the average amount for all orders for one user' do
    ...
    # Solution goes here
    # ------------------------------------------------------------
    average = User.joins(:orders).find("#{@user_3.id}").orders.average(:amount)
    # Expectation
